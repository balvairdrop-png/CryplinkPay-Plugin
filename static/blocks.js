(() => {
    // Guard: ensure WC Blocks globals exist before mounting
    if (!window.wc || !window.wc.wcSettings || !window.wc.wcBlocksRegistry || !window.wp || !window.wp.element) {
        return;
    }

    const settings = wc.wcSettings.getSetting('cryplink_data', {}) || {};
    const { registerPaymentMethod } = window.wc.wcBlocksRegistry;
    const { createElement, useState, useEffect } = window.wp.element;
    const decodeEntities = (window.wp && window.wp.htmlEntities && window.wp.htmlEntities.decodeEntities)
    ...
    const updateCoin = async (coin, selected = false) => {
        try {
            await apiFetch({
                path: '/cryplink/v1/update-coin',
                method: 'POST',
                headers: {
                    'X-WP-Nonce': (typeof cryplinkData !== 'undefined' && cryplinkData.nonce) ? cryplinkData.nonce : '',
                },
                data: {
                    coin: coin,
                    selected: selected
    ...
    const Content = ({ eventRegistration, emitResponse }) => {
                ...
                if (selectedCoin !== 'none') {
                    try {
                        const data = await apiFetch({
                            ...
                            data: {
                                coin: selectedCoin,
                                fiat: (wcSettings && wcSettings.currency && wcSettings.currency.code ? wcSettings.currency.code : (wc.wcSettings.getSetting('currency', {}).code || 'USD')).toLowerCase(),
                                value: cartTotals ? cartTotals.total_price / Math.pow(10, cartTotals.currency_minor_unit) : 0,
                            },
                        });
        ...
        const coins = Array.isArray(settings.coins) ? settings.coins : [];
        const options = coins.map((val) => {
            const img = val.logo;
        ...
        return createElement('div', { className: 'form-row form-row-wide cryplink-payment-selection' },
    ...
    const Label = ({ components }) => {
        ...
        return createElement(
            'div',
            {
                className: name + '-payment-gateway cryplink-payment-selection',
            },
            iconsComp,
            labelComp
        );
